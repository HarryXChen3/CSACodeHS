...
public class Exercise_10_2_7 {
    @SuppressWarnings({"ForLoopReplaceableByForEach", "InnerClassMayBeStatic"})
        ...
        public static int linearSearch(int[] array, int number)
        {
            ...
            for (int i = 0; i < array.length; i++)
            {
                if (array[i] == number)
                {
                    ...
                    return c; // the method returns as soon as the number is found
                }

                c++;
            }
            return c; // the code will get here if the number isn't found
        }
    }
}
